...
namespace MediaBrowser.Library.Playables
{
    ...
    public static class PlaybackControllerHelper
    {
        ...
        public static string BuildStreamingUrl(Media item, int bitrate)
        {
            ...
            var info = item.MediaSources != null && item.MediaSources.Any() ? new StreamBuilder().BuildVideoItem(new VideoOptions { DeviceId = Kernel.ApiClient.DeviceId, ItemId = item.ApiId, MediaSources = item.MediaSources, MaxBitrate = bitrate, SupportsDirectRemoteContent = true, Profile = profile }) : null;
            if (info != null)
            {
        ...
        public static bool CallPlayMedia(MediaCenterEnvironment mediaCenterEnvironment, Microsoft.MediaCenter.MediaType type, object media, bool queue)
        {
            ...
            //file = "d:\\antz (1998).mp4";
            //file = @"\\lounge\Media\Childrens\Films\Antz (1998).mp4";
